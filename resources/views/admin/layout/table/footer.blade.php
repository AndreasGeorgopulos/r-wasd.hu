<div class="row">
    <div class="col-sm-5">
        ...
        {{trans('Results')}}: {{$list->total()}},
{{trans('Items')}}: {{ (($list->perPage() * $list->currentPage()) - $list->perPage()) + 1 }} - {{$list->perPage() * $list->currentPage()}}
        </div>
    </div>
    <div class="col-sm-7">
        @if ($list->lastPage() > 1)
        <div class="dataTables_paginate paging_simple_numbers">
            <ul class="pagination">
                <li class="paginate_button btn-sm previous @if($list->currentPage() == 1) disabled @endif">
                    <a href="#" data-page="{{$list->currentPage() - 1}}">{{trans('Prev')}}</a>
                </li>
                ...
                <li class="paginate_button btn-sm next @if($list->currentPage() == $list->lastPage()) disabled @endif">
                    <a href="#" data-page="{{$list->currentPage() + 1}}">{{trans('Next')}}</a>
                </li>
            </ul>
        </div>
        @endif
