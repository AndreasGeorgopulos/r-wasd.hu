@foreach (config('app.languages') as $lang)
    ...
    <div class="tab-pane" id="{{$lang}}_data">
        ...
        <div class="form-group col-md-12">
            <label>{{trans('Lead')}}</label>
            <textarea name="translate[{{$lang}}][lead]" class="form-control tinymce">{{old('lead', !empty($translate->lead) ? $translate->lead : '')}}</textarea>
        </div>

        <div class="form-group col-md-12">
            <label>{{trans('Body')}}</label>
            <textarea name="translate[{{$lang}}][body]" class="form-control tinymce">{{old('body', !empty($translate->body) ? $translate->body : '')}}</textarea>
        </div>
